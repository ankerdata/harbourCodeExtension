...
function computeSemanticTokens(p: Provider): { data: number[]; absolute: number[][] } {
    let ret: number[][] = [];
    for (const info of p.funcList) {
        if ((info.kind === "local" || info.kind === "param") && info.nameCmp in p.references) {
...
describe("semantic tokens", () => {
    describe("procedures.prg", () => {
        ...
        it("flags the declaration site with modifier bit 0", () => {
            ...
            const localN = p.funcList.find((i) => i.name === "n" && i.parent === main)!;
        ...
        it("scopes tokens to their parent function (no cross-function bleed)", () => {
            ...
            for (const t of absolute) {
                ...
                const inHelper = t[0] >= helper.startLine && t[0] <= (helper.endLine ?? 0);
    ...
    describe("class.prg (no eligible tokens)", () => {
        ...
        it("does not emit tokens for DATA members", () => {
            ...
            const oCnt = p.funcList.find((i) => i.name === "oCnt" && i.parent === driver)!;
