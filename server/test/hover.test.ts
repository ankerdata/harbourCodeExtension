...
function findHoverInfo(p: Provider, word: string, atLine: number): HoverPayload | undefined {
    ...
    for (const info of p.funcList) {
        ...
        if ((info.kind === "local" || info.kind === "param") && info.parent) {
            ...
            if (info.parent.endLine !== undefined && info.parent.endLine < atLine) continue;
        }
...
function buildPayload(info: Info, p: Provider): HoverPayload {
    const payload: HoverPayload = { label: info.kind.toUpperCase() + " " + info.name };
    if (info.comment && info.comment.trim().length > 0) payload.comment = info.comment.trim();
