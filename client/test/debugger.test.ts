...
function makeEvaluateResponse(expression: string): DebugProtocol.EvaluateResponse {
    return {
        type: "response",
        request_seq: 1,
        success: true,
...
describe("debugger.getVariableFormat", () => {
    ...
    it("unescapes \\$\\n and \\$\\r inside C (string) values", () => {
        ...
        session.getVariableFormat(dest, "C", "line1\\$\\nline2\\$\\rend", "value", "LOC:1:1:");
    ...
    it("chains evaluateName for nested arrays: outer[i][j]", () => {
        ...
        session.getVariableFormat(leaf, "N", "42", "value", "LOC:1:1:1:2", innerIdx!);
...
describe("debugger.evaluateRequest", () => {
    it("queues the response, normalizes colons in the expression, and emits EXPRESSION", () => {
        ...
        expect((response.body as { result: string }).result).toBe("AClone({ 'a':1 })");
    });
});
