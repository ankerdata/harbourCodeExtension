...
# 1.1.3
 - **Debugger** fixed multi-threaded debugging UX where hitting a breakpoint on a non-main thread silently paused the main thread "wherever it happens to be" and stepping the breakpoint thread also stepped main, with VS Code's focus ricocheting between threads. Three adapter-side bugs combined to make MT debugging feel like an all-stop debugger even though the runtime was per-thread since 1.1.0: `StoppedEvent.allThreadsStopped` was never set (VS Code's UI fell into all-stop semantics); `ContinueResponse.body.allThreadsContinued` was unset (DAP-spec default `true` told VS Code every thread continued, so it re-snapshotted unrelated threads on the next stop); and newly-connected threads never got an auto-`GO`, so `dbg_lib`'s fallback sleep+`STOP:step` branch made every spawned worker appear paused on its first line. Now: every `StoppedEvent` carries `allThreadsStopped: false`, every `ContinueResponse` carries `allThreadsContinued: false`, and `acceptThreadSocket` auto-`GO`s non-main threads with a one-shot swallow of the racing initial `STOP:step` (so the runtime race between our `GO` and `dbg_lib`'s 100 ms sleep doesn't surface as a phantom paused state). Closes #34.
 - **Tests** added 8 tests pinning the corrected `StoppedEvent` / `ContinueResponse` / auto-`GO` shape and the per-thread one-shot swallow
